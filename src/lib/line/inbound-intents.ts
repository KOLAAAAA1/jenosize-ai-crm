...
const CONTACT_TEAM_REPLY =
  'รับเรื่องแล้วครับ ✓ ทีมงานจะติดต่อกลับโดยเร็วที่สุด';
const INQUIRY_START_REPLY =
  'ยินดีให้ข้อมูลครับ 🙏 รบกวนแจ้ง\n (1) สินค้า/บริการที่สนใจ\n (2) ความต้องการ\n (3) งบประมาณโดยประมาณ \nแล้วทีมขายจะติดต่อกลับพร้อมรายละเอียดครับ';
const INQUIRY_CAPTURE_REPLY =
  'ได้รับข้อมูลแล้วครับ ✓ ทีมขายจะติดต่อกลับพร้อมรายละเอียดโดยเร็วครับ';
...
): { kind: IntentKind; replyText: string } {
  const t = text.trim();
  if (t === CONTACT_TEAM_KEYWORD)
    return { kind: 'contact_team', replyText: CONTACT_TEAM_REPLY };
  if (t === INQUIRY_KEYWORD)
    return { kind: 'inquiry_start', replyText: INQUIRY_START_REPLY };
  if (pendingIntent === 'AWAITING_INQUIRY')
    return { kind: 'inquiry_capture', replyText: INQUIRY_CAPTURE_REPLY };
  return { kind: 'none', replyText: '' };
}

function leadTitle(text: string): string {
  const oneLine = text.replace(/\s+/g, ' ').trim();
  return `สอบถามผ่าน LINE — ${
...
async function handleOne(
  ...
  if (intent.kind === 'contact_team') {
    ...
    if (leadId) {
      await db.activity.create({
        data: {
          ...
          userId: null,
          type: 'NOTE',
          body: 'ลูกค้าขอให้ทีมงานติดต่อกลับ (LINE rich menu)',
          metadata: { via: 'line_rich_menu', lineUserId: userId },
        },
      });
    }
    ...
    await db.contact.update({
      where: { id: contact.id },
      data: { pendingIntent: 'AWAITING_INQUIRY' },
    });
  } else {
    ...
    const created = await db.$transaction(async (tx) => {
      ...
      await tx.activity.create({
        data: {
          ...
          userId: null,
          type: 'LINE_IN',
          body: `สอบถามข้อมูลผ่าน LINE: ${text}`,
          metadata: { via: 'line_rich_menu_inquiry', lineUserId: userId },
        },
      });
      return true;
    });
  ...
  try {
    ...
    logger.warn('line.intents.reply_error', {
      error: err instanceof Error ? err.message : 'unknown error',
    });
  }
