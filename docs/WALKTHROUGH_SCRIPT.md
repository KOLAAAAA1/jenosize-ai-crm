...
## บทพากย์ภาษาไทย (Thai narration script — อ่านออกเสียงขณะบันทึก)

บทพากย์ภาษาไทยพร้อมอ่าน สอดคล้องกับแต่ละฉากด้านบน อ่านหนึ่งย่อหน้าต่อหนึ่งฉากขณะทำแอ็กชันบนหน้าจอ (ความยาวรวมประมาณ 3–5 นาที)

**0:00–0:30 — บริบทและการออกแบบ**
> สวัสดีครับ นี่คือระบบ AI CRM สำหรับทีมขายประมาณ 20 คน ดูแลผู้ติดต่อราวสองพันราย และดีลที่กำลังดำเนินการอยู่ประมาณสามร้อยดีล ผมเลือกพัฒนาเป็นแอป Next.js แบบ full-stack ตัวเดียว ใช้ Prisma และ PostgreSQL เพื่อให้เวลาที่มีจำกัดทุ่มไปกับความลึกของฟีเจอร์ การเก็บข้อมูลถาวร ความปลอดภัยของ AI และการเชื่อมต่อ LINE แทนที่จะเสียเวลากับการต่อระบบหลายบริการ

**0:30–1:30 — ฟลูว์หลักของ CRM**
> เข้าสู่ระบบด้วยบัญชีเดโมผ่านเซสชันแบบ HttpOnly นี่คือรายการลีด รองรับการค้นหา การกรอง และการแบ่งหน้าจากฝั่งเซิร์ฟเวอร์ เมื่อเปิดลีดขึ้นมา ผมลองเปลี่ยนสถานะ ซึ่งเป็นเซอร์วิสกลางที่ห่อด้วย server action มันจะอัปเดตลีดและบันทึกกิจกรรมการเปลี่ยนสถานะที่แก้ไขไม่ได้ลงไทม์ไลน์ หลังจากรีเฟรชหน้า ข้อมูลยังคงอยู่ ไม่ได้เก็บไว้แค่ในหน่วยความจำ และหน้า Pipeline ก็แสดงข้อมูลชุดเดียวกันโดยแยกตามสถานะ

**1:30–2:30 — ขอบเขตของ AI Copilot**
> AI copilot จะรวบรวมบริบทของลีด แล้วสร้างบทสรุป คะแนนคุณภาพลีดพร้อมเหตุผล และการกระทำถัดไปที่ควรทำ ถ้าโมเดลไม่พร้อมใช้งาน ระบบจะถอยไปใช้การให้คะแนนแบบกำหนดกฎ ซึ่งจะระบุไว้ชัดเจน จุดสำคัญของการออกแบบคือ ผลลัพธ์จาก AI จะไม่กลายเป็นข้อมูลจริงใน CRM โดยอัตโนมัติ มันถูกเก็บเป็นข้อเสนอแนะ และต้องให้คนกดยอมรับหรือปฏิเสธ เมื่อกดยอมรับ คะแนนจะถูกบันทึกลงลีด ส่วนข้อความ LINE ที่ร่างไว้ก็จะถูกบันทึกเป็นฉบับร่าง ไม่ได้ส่งออกเอง

**2:30–3:30 — ความปลอดภัยของ LINE OA**
> ข้อความ LINE ขาเข้าจะวิ่งเข้ามาที่ webhook นี้ ระบบจะตรวจสอบลายเซ็นกับเนื้อหาดิบของ request ก่อนแปลงข้อมูล และกันเหตุการณ์ซ้ำด้วย event id และ message id เพื่อไม่ให้เขียนข้อมูลซ้ำแม้จะมีการส่งซ้ำ ข้อความที่จับคู่กับผู้ติดต่อได้จะไปปรากฏบนไทม์ไลน์ของลีด ส่วนขาออกต้องผ่านการอนุมัติก่อน เมื่อกดอนุมัติ ระบบจะส่งข้อความแบบ push พร้อม retry key เพราะการอนุมัติโดยคนอาจเกิดขึ้นหลังจากที่ reply token ของ webhook หมดอายุแล้ว

**3:30–4:30 — หลักฐานและการส่งมอบงาน**
> เทสต์อัตโนมัติที่กำหนดไว้ครอบคลุมฟลูว์หลักของ CRM เส้นทาง fallback ของ AI และการตรวจสอบลายเซ็นกับการกันซ้ำของ LINE webhook รวมถึงการอนุมัติขาออก ผมรัน pnpm test สดให้ดูจำนวนที่ผ่านจริง ในส่วนเอกสารมี README ที่ครอบคลุมการติดตั้ง การ deploy บันทึก API และแนวทางการมอนิเตอร์ พร้อมทั้งบันทึกการใช้งาน AI และเช็กลิสต์การส่งมอบงาน

**4:30–5:00 — ขั้นตอนถัดไปสำหรับ production**
> สิ่งที่ตั้งใจเลื่อนไว้สำหรับ production ได้แก่ ระบบยืนยันตัวตนและการจัดการสิทธิ์จริงแทนบัญชีเดโม การรับข้อความ LINE แบบคิวสำหรับทราฟฟิกสูง การรวมศูนย์ log และการแจ้งเตือน การเพิ่มชุดทดสอบ AI ใน CI และนโยบายการเก็บและลบข้อมูลส่วนบุคคลสำหรับข้อมูลลูกค้าจริง ขอบคุณครับ
