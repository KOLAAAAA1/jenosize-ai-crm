...
const id = (prefix: string, i: number) => `${prefix}_${String(i).padStart(5, "0")}`;
...
const ROMAN_NAMES = [
  "somchai", "somsri", "nattapong", "pimchanok", "arthit", "kanya",
  "thanawat", "napaporn", "chatchai", "ratana", "worawut", "sunisa",
];
const EMAIL_DOMAINS = ["gmail.com", "hotmail.com", "outlook.co.th", "yahoo.co.th"];
const cleanEmail = (i: number, domain: string) => `${ROMAN_NAMES[i % ROMAN_NAMES.length]}.${i}@${domain}`;
...
const COMPANY_WORDS = ["เทรดดิ้ง", "การช่าง", "พาณิชย์", "กรุ๊ป", "อุตสาหกรรม", "เทคโนโลยี", "อินเตอร์เนชั่นแนล", "โฮลดิ้ง"];
const thaiCompanyName = () => `บริษัท ${faker.person.lastName().split("-")[0]}${pick(COMPANY_WORDS)} จำกัด`;
...
async function main() {
  ...
  const fixedEmails = ["admin@jenosize.demo", "manager@jenosize.demo", "sales@jenosize.demo"];
  const users = Array.from({ length: N_USERS }, (_, i) => {
    const role = i === 0 ? UserRole.ADMIN : i <= 3 ? UserRole.MANAGER : UserRole.SALES;
    return {
      id: id("usr", i),
  ...
  const companies = Array.from({ length: N_COMPANIES }, (_, i) => ({
    ...
    notes: faker.helpers.maybe(() => faker.company.catchPhrase(), { probability: 0.5 }) ?? null,
    createdAt: dateBetween(ONE_YEAR_AGO, NOW),
  }));
  ...
  const makeLineId = () => {
    let v: string;
    do {
      v = "U" + faker.string.hexadecimal({ length: 32, casing: "lower", prefix: "" });
  ...
  const leads = Array.from({ length: N_LEADS }, (_, i) => {
    ...
    return {
      ...
      scoreReason: scored ? faker.helpers.arrayElement(["Strong budget + timeline fit", "Engaged decision maker", "Clear pain, mid-tier budget", "Warm inbound, needs nurture"]) : null,
      createdAt,
  ...
  for (const lead of leads) {
    ...
    for (let k = 1; k < count; k++) {
      ...
      const type = pick([ActivityType.CALL, ActivityType.EMAIL, ActivityType.NOTE, ActivityType.STAGE_CHANGE]);
      activities.push({
        id: id("act", actCounter++),
        leadId: lead.id,
        userId: faker.helpers.maybe(() => lead.ownerId, { probability: 0.85 }) ?? null,
        type,
        body:
          type === ActivityType.STAGE_CHANGE
            ? `Stage moved to ${lead.stage}.`
            : faker.lorem.sentence(),
        metadata: type === ActivityType.STAGE_CHANGE ? { to: lead.stage } : Prisma.JsonNull,
        createdAt: cursor,
      });
    }
  }
  ...
  for (const contact of lineContacts) {
    ...
    for (let t = 0; t < turns; t++) {
      ...
      messages.push({
        ...
        status: inbound ? MessageStatus.RECEIVED : pick([MessageStatus.SENT, MessageStatus.APPROVED, MessageStatus.DRAFT]),
        body: inbound ? faker.lorem.sentence() : `Thanks for reaching out! ${faker.lorem.sentence()}`,
        createdAt: cursor,
      });
    }
  }
  ...
  const suggestions: Prisma.AiSuggestionCreateManyInput[] = suggestionLeads.map((lead, i) => {
    ...
    return {
      ...
      payload: {
        ...
        nextAction: {
          action: pick(["Book a discovery call", "Send pricing proposal", "Follow up in 3 days"]),
  ...
  const webhookEvents: Prisma.WebhookEventCreateManyInput[] = Array.from({ length: 30 }, (_, i) => ({
    id: id("evt", i),
    provider: "LINE",
    providerEventId: `wh_${faker.string.alphanumeric(24)}`,
    signatureValid: true,
    rawPayload: { events: [{ type: "message", message: { type: "text", text: faker.lorem.sentence() } }] },
